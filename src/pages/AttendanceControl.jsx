...
const AttendanceControl = () => {
    ...
    const [editReason, setEditReason] = useState(''); // Nuevo estado para el motivo
    ...
    const handleManualLog = async (e) => {
        ...
        if (!editReason.trim()) {
            alert("Es OBLIGATORIO indicar un 'Motivo del ajuste' para mantener la auditoría.");
            return;
        }
        ...
        try {
            ...
            const { error } = await supabase.from('rrhh_attendance_logs').insert({
                employee_id: selectedDay.employee_id,
                type: newLogType,
                timestamp: logDateTime.toISOString(),
                device_info: 'CORRECCION_MANUAL',
            ...
            const { data: logs } = await supabase.from('rrhh_attendance_logs').select('*')
                ...
                .gte('timestamp', `${selectedDay.work_date}T00:00:00`)
                .lte('timestamp', `${selectedDay.work_date}T23:59:59`)
                .order('timestamp', { ascending: true });

            setDailyLogs(logs || []);
            fetchData();
            setNewLogTime('');
            setEditReason(''); // Limpiar motivo
    ...
    const handleDeleteLog = async (logId) => {
        const confirmation = window.prompt("⚠️ ACClÓN DESTRUCTIVA ⚠️\n\nEstás a punto de borrar un registro de asistencia. Esto dejará un hueco en la auditoría.\n\nPara confirmar, escribe 'BORRAR' en el campo de abajo:");

        if (confirmation !== 'BORRAR') {
            if (confirmation !== null) alert("Cancelado: La palabra de confirmación no coincide.");
            return;
        }
                        ...
                        <div className="p-5 space-y-5">
                            ...
                            <div className="border rounded-lg overflow-hidden">
                                ...
                                <div className="divide-y divide-gray-100">
                                    ...
                                    {dailyLogs.map(log => {
                                        const isModified = log.is_modified || log.device_info === 'CORRECCION_MANUAL';
                                        return (
                                            <div key={log.id} className={`flex justify-between items-center p-3 hover:bg-slate-50 transition-colors ${isModified ? 'bg-amber-50/50' : ''}`}>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {isModified && (
                                                        <div className="group relative flex items-center">
                                                            ...
                                                            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 w-64 p-2 bg-slate-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-10">
                                                                <p className="font-bold mb-1">Registro Modificado Manualmente</p>
                                                                <p>Motivo: {log.edit_reason || 'No especificado'}</p>
                                                                {log.edited_by && <p className="mt-1 text-slate-400 text-[10px]">ID: {log.edited_by}</p>}
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="text-right">
                                                        <span className="block text-xs text-gray-400" title={log.device_info}>{getDeviceLabel(log.device_info)}</span>
                                                    </div>
                                                    <button onClick={() => handleDeleteLog(log.id)} className="text-gray-300 hover:text-red-500 p-1" title="Borrar Registro">🗑️</button>
                                                </div>
                                            </div>
                                        );
                                    })}
                            ...
                            <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                                <p className="text-xs font-bold text-slate-500 mb-3 uppercase flex items-center gap-2">
                                    <Info size={14} />
                                    Corrección Manual (Auditoría)
                                </p>
                                <form onSubmit={handleManualLog} className="flex flex-col gap-3">
                                    <div className="flex gap-2">
                                        <select value={newLogType} onChange={(e) => setNewLogType(e.target.value)} className="border border-slate-300 p-2 rounded-lg text-sm bg-white font-medium focus:ring-2 focus:ring-blue-500 outline-none">
                                        ...
                                        <input
                                            ...
                                            required
                                            className="border border-slate-300 p-2 rounded-lg text-sm flex-1 focus:ring-2 focus:ring-blue-500 outline-none"
                                        />
                                    </div>
                                    <input
                                        type="text"
                                        ...
                                        placeholder="Motivo del ajuste (Ej: Olvido marcar, Falla reloj)"
                                        className="w-full border border-slate-300 p-2 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                                    />
                                    <button
                                        type="submit"
                                        className="w-full bg-slate-800 text-white px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-slate-700 shadow-sm transition-colors flex justify-center gap-2"
                                    >
                                        Guardar Ajuste
                                    </button>
                                </form>

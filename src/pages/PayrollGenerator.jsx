...
const PayrollGenerator = () => {
    ...
    useEffect(() => {
        const init = async () => {
            const empData = await supabase.from('rrhh_employees').select('id, first_name, last_name, rut').order('last_name');
    ...
    useEffect(() => {
        const loadIndicators = async () => {
            const { data } = await supabase.from('rrhh_payroll_parameters').select('*').eq('period_date', `${selectedMonth}-01`).single();
    ...
    const prepareBulkDownload = async () => {
        ...
        try {
            const { data, error } = await supabase.from('rrhh_payrolls').select(`*, employee:employee_id(*, job:job_id(name), pension:pension_provider_id(name), health:health_provider_id(name)), items:rrhh_payroll_items(*)`).eq('period_date', `${selectedMonth}-01`).eq('is_closed', true);
            if (error) throw error;
